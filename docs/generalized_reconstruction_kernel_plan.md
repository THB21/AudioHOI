...
- chair/stick 仍未由同一 generic factor executor 写 accepted outputs；line contact 可继续保留为
  nonblocking compatibility mechanism。
- chair 的 pairprop/contact evidence 已有只读 diagnostics；two-point contact chord
  initializer 和 articulated joint propagation 已提升为通用 core contract；canonical Stage4
  已改用 current-run `object_pose_init.csv`，不再使用 `rebuilt_from_mainline_saved2d`
