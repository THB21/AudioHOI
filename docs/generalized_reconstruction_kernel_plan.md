...
| `refactor/migrate-chair-case` | chair URDF + articulated DOF + two-hand contact 迁移 | in_progress | chair diagnostics gate 与通用 two-point `RigidCorrespondenceInitializer` 已建立；URDF/factor executor 未迁完前 solver gap 继续 blocking |
...
- chair 的 pairprop/contact evidence 已有只读 diagnostics；two-point contact chord
  initializer 已提升为通用 core contract；canonical `rebuilt_from_mainline_saved2d`
  seed 仍使 `semantic_graph_solver_private` 保持 blocking。
