...
| `refactor/migrate-chair-case` | chair URDF + articulated DOF + two-hand contact 迁移 | in_progress | chair diagnostics、two-point `RigidCorrespondenceInitializer`、`ArticulatedKinematicProvider`、required Factor IR、required residual evaluator、isolated candidate attempt/residual coverage 外壳已建立；generic executor 未产出 candidate pose 前 solver gap 继续 blocking |
...
- chair 的 pairprop/contact evidence 已有只读 diagnostics；two-point contact chord
  ...
  generic executor 消费。chair candidate attempt 现在只允许写 safe manifest，不允许写
  `object_pose.csv` 等 accepted outputs。chair residual loop 已开始复用 core
  `FactorResidualEvaluator`；required residual blocks 已覆盖，但还没有形成可独立产出
  pose 的 generic executor。candidate sandbox 现在写入 safe residual coverage manifest，
  但仍不写 accepted outputs。
