...
| `refactor/generic-sequence-solver` | 通用初始化、单帧/序列求解、deterministic attempt provenance | in_progress | sequence problem/diagnostics/candidate sandbox 已建立；115 tests；不读取 baseline pose；不写 accepted 输出 |
...
| 2026-07-26 | in_progress | 增加 sequence solver shadow diagnostics；basketball/football 标记 future-shadow-ready，mug/chair/stick 被机制 gap 阻断 | `docs/generic_sequence_solver_plan.md` |
