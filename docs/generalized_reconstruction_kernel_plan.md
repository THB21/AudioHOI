...
| `refactor/migrate-chair-case` | chair URDF + articulated DOF + two-hand contact 迁移 | in_progress | chair pairprop/contact diagnostics gate 已建立；semantic 2D、contact P90、freeze、render 不退化后才能关闭 solver gap |
...
- chair 的 pairprop/contact evidence 已有只读 diagnostics；canonical
  `rebuilt_from_mainline_saved2d` seed 仍使 `semantic_graph_solver_private` 保持 blocking。
