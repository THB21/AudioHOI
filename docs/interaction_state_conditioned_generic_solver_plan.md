...
| `refactor/interaction-state-production` | in_progress | 已引入生产级 `InteractionStateIR` 和 typed `AudioEventIR`，并将 Measurement / Audio / Contact / Interaction hashes 纳入 `SequenceProblemContract`；已新增通用 factor activation ledger、`CompiledFactor` runtime contract、generic runtime / attempt / residual execution 边界，以及按 residual capability 解析显式输入的 case-independent provider boundary。contact/audio artifact 按能力路径解析，不依赖 case：mug 现为 240 帧 active/persistent grasp，chair 为 125 帧 grasp + 1 release，且五 case 共享 `results/events/audio_events.csv` 能进入正式 contract。audio factor 只有在 audio peak 与 inferred impact/release transition 同时存在时 active；chair 只激活 frame 145 的 release event，而非把 8 个峰值全部当 contact。world-space contact 已具备 sphere、capsule、periodic rigid semantic feature cloud、articulated semantic feature cloud 四类 GeometryProvider。stick / mug 的真实 geometry/contact dry-run 已 `skipped=0`；chair 的 typed line reprojection、contact、joint、temporal、pose/reg 与真实 audio alignment 已执行。chair 原 legacy `E_visual` audit 因子已由 822 条 `Line2DMeasurement` 替换；无 typed floor-plane evidence 的伪 `E_support` 已删除。旧 contact-chord gauge 实际是 two-palm contact 固定 chord 后复用多线段 reprojection解 twist，因此重复的 gauge shadow 也已删除。通用 `PlaneSurface` support/penetration residual contract 已建立，但不会用 canonical pose 反推零残差地面。`GenericSequenceExecutor.solve()` 已实际消费 factor-id keyed runtime program；同一 executor 直接消费 `StateSpec`，以 SO(3) tangent、periodic wrap、schema joint/scale bounds 和不可观测旋转冻结构造优化变量。typed input builder 自动生成 row-level factor-frame dependencies 和严格校验的 sparse Jacobian；`SequenceProblemFactory` 进一步统一 configured factor selection、residual closure、StateSpec parameterization、typed initializer application 与 sparse dependency assembly。冻结的 chair parity weight、sigma、unit、state scales 与 provenance 现由 `CompiledFactor.runtime_config` 进入 residual plan，求解 factor 集也由“具有 production runtime config”这一通用规则选择，不再由 chair caller 手列或手传数值。逐帧 `active/downweighted/inactive` intervals 和有限 weight tiers 已从 interaction timeline 编译进 configured factors，并由 line/contact/temporal residual 真实消费；未配置 factors 仍保持旧 shadow hash。world-space contact sample 现可逐条保留 typed target feature identity；`StateSpecRigidCorrespondenceInitializer` 与 typed assembler 直接从 `ContactConstraintIR + HumanSiteMeasurement + StateSpec` 生成两点刚体 hypothesis，保留其余 scale/joint/phase state，不读取 case 或旧 solved pose。正式 chair attempt 的 status 已绑定 initializer kind 与 ledger SHA-256，100 eval 将 `9343.7383 → 6423.2060`；line RMSE `27.92px` 优于 legacy reference `34.69px`，contact median/p90/max 为 `0.02186/0.05318/0.10349m`。五帧 object-only overlay 与 initializer provenance 已记录。整个路径不识别 object/case，GVHMR 仍只读，accepted output 尚未写入。 |
| 下一步 | pending | contact tail 的主因已确认是 Stage-3 seed 距接触流形过远、`soft_l1` tail 饱和，以及 runtime sample 丢失 per-sample target feature identity；不是 coverage 缺帧或 activation 错误。case-independent typed assembler 现已把 `ContactConstraintIR + HumanSiteMeasurement + StateSpec` 编译为带完整 constraint/site/feature provenance 的 rigid hypothesis ledger，并把 initializer kind/hash绑定到 `SequenceOptimizationProblem` 和 isolated attempt status。五 case capability audit 为：chair `125/125` 帧 seeded；mug 240 帧明确跳过为 single-point rotation-unobservable；ball/stick 不误入 LocalXYZ two-point initializer。正式 `SequenceProblemFactory` 现统一负责 typed hypothesis application、production-configured factor 选择、residual input closure、StateSpec parameterization 与 row-level sparse dependencies；下一增量是让现有 chair production preparation 调用该 factory，并把当前 validation caller 中的 semantic feature registration 改为 asset/geometry descriptor 驱动。剩余 onset frames `20-24` 的 max gap 约 `0.10m` 继续作为 blocking hard gate，不通过无限 resume、改权重或 chair 专用 freeze 掩盖。保持严格 object-only，不写 accepted、不引入 case dispatcher。 |
