...
| 下一步 | pending | 把 full-sequence solve result 原子写入通用 isolated attempt artifacts，允许从未收敛 attempt 的 object state 继续求解；随后完成 chair pose/render regression。calibrated floor plane 进入 typed scene geometry 后再启用通用 support factor。保持严格 object-only，不写 accepted、不引入 case dispatcher。 |
