...
| `refactor/interaction-state-production` | in_progress | 前三项基础已完成：chair preparation、descriptor 驱动的 geometry/semantic mapping，以及 mug/stick/chair 的统一 executable candidate/publisher。三类对象现在均由 `prepare_capability_object_problem()` 按 geometry capability 组装 `SequenceProblemFactory` 输入，再由唯一 `GenericSequenceExecutor` 求解并进入唯一 `AcceptedObjectOutputPublisher`。mug 从 observation-derived body/axial-phase seed 初始化，不读历史 solved pose；stick 从两条 typed `LineS` 与只读 GVHMR palm site 恢复 capsule root SE(3)，不读 `line_contact_lock` pose；chair继续从 articulated asset descriptor/URDF/typed contacts初始化。publisher 已执行 solver convergence、objective、contact 和 reprojection normalized RMSE hard gates；未通过时只写 isolated attempt 与 `generic_object_pose_candidate.csv`，不会写 canonical output。GVHMR仍是只读 object-side measurement，未优化人体、未修改或编排 downstream human pipeline。 |
| 下一步 | pending | 继续做生产质量收敛与回归，而不是增加新机制：用正常 evaluation budget 跑三类统一 attempt，补全 object-only projection/contact/render hard metrics并处理现有 gate failure；在 gate 通过前不 promotion canonical output。之后才进入 held-out zero-shot object examples。权重变化只允许由 interaction/VLM gate 在冻结的通用 tiers 内选择，不恢复 case-name solver或对象专用优化器。 |
