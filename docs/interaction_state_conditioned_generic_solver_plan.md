...
| 下一步 | pending | 将 legacy parity 中已冻结的数值 weight / sigma / activation 作为 `CompiledFactor` runtime configuration 正式输入，不再由 chair driver 手传；随后用同一配置重建 attempt 并以 contact-tail hard gate 判断是否可做 accepted promotion。calibrated floor plane 进入 typed scene geometry 后再启用通用 support factor。保持严格 object-only，不写 accepted、不引入 case dispatcher。 |
