...
| 下一步 | pending | canonical chair preparation 现已通过 `audiohoi` runtime 调用正式 `SequenceProblemFactory`：从当前 Stage-3 state、typed Measurement/Contact、只读 GVHMR sites 生成三个 production-configured factors、1263 个 sparse dependencies 和 125 帧 typed rigid hypotheses。articulated semantic features、part mapping 与 kinematic rules 已迁入 asset descriptor；关节 origin/axis 从 descriptor 指向的 URDF 读取，contact points 从 typed LocalXYZ 注册，core 不读取 chair 名称。下一增量收束 mug、stick、chair 的 executable candidate 到同一 `GenericSequenceExecutor` 与单一 object publisher；hard gate 未通过时 publisher 必须拒绝 canonical promotion。chair onset max gap 约 `0.10m` 仍是 blocking gate，不通过无限 resume、人工 case 权重或专用 freeze 掩盖。保持严格 object-only。 |
