...
| `refactor/interaction-state-production` | in_progress | 已引入生产级 `InteractionStateIR` 底座；暂不改变 accepted output、loss、阈值或求解路径。 |
| 下一步 | pending | 让 `InteractionStateIR` 进入 factor activation / compiled factor ledger，而不是继续增加 case dispatcher。 |
