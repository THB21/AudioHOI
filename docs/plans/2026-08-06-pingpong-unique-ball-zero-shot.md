...
- `output/pingpong_right_hand_driven_candidate/paddle_pose.csv` — observed-rigid paddle pose with an exact immutable `right_hand` palm-to-handle constraint and hand-driven rotation
- `samples_known_object/14_pingpong_wall/results/renders/pingpong_right_hand_driven_candidate/with_human/camera3d.mp4` — fixed-view camera-space 3D containing the read-only GVHMR skeleton, Articraft paddle and sphere
...
- 2026-08-06: persistent-grasp correction — the prior observed-rigid builder used the right palm only as weak depth/image evidence and allowed a 160+ mean handle gap of about 0.23 m. Its first exact-pivot revision was also rejected: independently rotating around the palm to fit every SAM2 centroid reduced image error but caused nonphysical steps up to about 37°/frame. The current candidate fixes `grasp_site_id=right_hand` for all 240 frames, forbids hand switching, drives orientation from smoothed read-only right-palm joint axes, and limits mask evidence to a bounded smooth correction. Maximum grasp gap is numerical zero; rotation-step median/p95/max are 1.60°/2.58°/3.22° per frame under a 4° hard validator. Human state remains read-only. A fixed-view skeleton render and a two-object overlay are published for review.
