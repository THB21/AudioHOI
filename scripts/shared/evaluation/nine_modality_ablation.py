...
PIPELINE_CASE_NAMES = {
    "pingpong": "pingpong_wall",
    "suitcase": "suitcase_drag",
}
PUBLIC_MODE_NAMES = {
    "visual_vlm_only": "vlm",
    "visual_vlm_plus_audio": "vlm_audio",
}
INPUT_ROOT = REPO / "final_result/nine_case_audio_ablation_inputs"
RENDER_VIEWS = {
...
PINGPONG_HUMAN_SITES = INPUT_ROOT / "pingpong/human_sites_hamer.csv"
WORLD_MESHES = {
    "basketball": (REPO / "assets/object_meshes/basketball.glb", False),
    "football": (REPO / "assets/object_meshes/football.glb", False),
    "mug": (REPO / "assets/object_meshes/mug_combined.obj", False),
    "chair": (REPO / "assets/object_meshes/chair_original_visuals.glb", True),
    "stick": (REPO / "assets/object_meshes/stick_original_visuals.glb", True),
    "back_view_basketball": (REPO / "assets/object_meshes/basketball.glb", False),
    "suitcase": (REPO / "assets/object_meshes/suitcase_extended_handle_contrast.glb", True),
}
HAMER_BODY_PARAMS = {
...
FROZEN_CONTROLLED_POSES = {
    (case, mode): INPUT_ROOT / case / f"{PUBLIC_MODE_NAMES[mode]}.csv"
    for case in CASES
    for mode in PUBLIC_MODE_NAMES
}


def pipeline_case_name(case: str) -> str:
    return PIPELINE_CASE_NAMES.get(case, case)
...
def executable(name: str, python: Path) -> Path:
    beside_python = python.resolve().parent / name
...
def ensure_h264(path: Path, python: Path) -> None:
    """Normalize a rendered MP4 to the browser-portable H.264 release codec."""
    ffprobe = executable("ffprobe", python)
    ...
    if streams and streams[0].get("codec_name") == "h264":
        return
    target = path.with_name(f"{path.stem}.h264.tmp.mp4")
    subprocess.run(
        [
            str(executable("ffmpeg", python)), "-y", "-i", str(path), "-an",
...
def load_profile(case: str) -> dict:
    import yaml
    path = (
        REPO / "scripts/shared/generic_contact_pipeline/configs/cases"
        / f"{pipeline_case_name(case)}.yaml"
    )
...
def render_world(case: str, mode: str, args: argparse.Namespace) -> Path:
    ...
    if not sample.is_absolute():
        sample = REPO / sample
    trajectory = FROZEN_CONTROLLED_POSES[(case, mode)]
    ...
    out.mkdir(parents=True, exist_ok=True)
    cmd = [
        str(args.python), str(WORLD_RENDERER),
        ...
        "--output-width", "1920",
        "--output-height", "1080",
        "--body-model-root", str(args.body_model_root),
    ]
    ...
    cmd.extend(["--body-params-pkl", str(hamer_params)])
    ...
    if case == "mug":
        cmd.extend(["--object-pose-csv", str(trajectory)])
    elif case == "pingpong":
...
def run_world_only(selected: list[str], args: argparse.Namespace) -> None:
    ...
    if args.plan:
        plan = {
            case: {
                PUBLIC_MODE_NAMES[mode]: {
                    "trajectory": repo_relative(FROZEN_CONTROLLED_POSES[(case, mode)]),
                    "body_parameters": repo_relative(HAMER_BODY_PARAMS[case]),
                    "output": repo_relative(
                        root / case / PUBLIC_MODE_NAMES[mode] / "world.mp4"
                    ),
                }
    ...
    for case in selected:
        ...
        for mode in ("visual_vlm_only", "visual_vlm_plus_audio"):
            ...
            ensure_h264(world, args.python)
            arms[PUBLIC_MODE_NAMES[mode]] = repo_relative(world)
        comparison = compose_world_comparison(case, executable("ffmpeg", args.python))
        manifest[case] = {**arms, "comparison": repo_relative(comparison)}
...
def main(default_mode: str | None = None) -> None:
    ...
    parser.add_argument("--python", type=Path, default=Path(sys.executable),
                        help="Python executable with the GVHMR rendering dependencies")
    parser.add_argument(
        "--body-model-root",
        type=Path,
        default=REPO / "scripts/third-party/GVHMR/inputs/checkpoints/body_models",
        help="directory containing smplx/SMPLX_NEUTRAL.npz",
    )
    ...
    if args.mode == "both" and not args.plan and not failed and not args.no_compare:
        ...
        ffmpeg = executable("ffmpeg", args.python)
        comparison_manifest = {
