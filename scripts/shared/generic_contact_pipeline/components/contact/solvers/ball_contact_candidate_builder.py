...
from scripts.shared.generic_contact_pipeline.components.contact.solvers.ball_contact_candidate_utils import (
    LEFT_HAND_KEY,
    RIGHT_HAND_KEY,
    LEFT_FOOT_KEY,
    RIGHT_FOOT_KEY,
...
def main() -> None:
    ...
    # proxy geometry gives absolute measurements, so use absolute thresholds and
    # interval peaks here -- don't let depth/support percentiles promote far human
    # gaps into anchor contacts.
    ...
    if anchor_event_mode == 'interval_peak':
        # proxy proximity is the base; object/audio motion can open a wider interval
        motion_gate = (motion_score >= 0.80) | (audio_support >= 0.20)
        impulse_state = (min_gap <= 52.0) & motion_gate
        # proximity alone isn't a state for impulse contacts -- an object resting
        # near a body part after the action must not become a human anchor
    ...
    if anchor_event_mode == 'interval_peak':
        # use the object's own bottom/support proxy peak. GVHMR floor bias makes
        # the absolute support-plane gap unreliable, so it's diagnostic only here.
    ...
    if anchor_event_mode == 'interval_peak':
        if contact_part_policy == 'feet':
            # localize by foot-object proximity; image height can shift the event
            # to a nearby flight frame
            anchor_event_index = build_peak_index_from_selector(anchor_state, anchor_score, 'max')
        else:
            # pick the highest object point in each interval (smaller v = higher up)
