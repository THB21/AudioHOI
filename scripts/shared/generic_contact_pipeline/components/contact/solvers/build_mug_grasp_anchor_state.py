...
Turns per-frame Articraft contact diagnostics into a state track separating
confirmed hand-handle grasp anchors, hidden/occluded frames that reuse the
previous anchor, rim drinking contacts that must not overwrite the hand anchor,
and debug/misaligned/no-contact frames.

Conservative by design: only frames the M13 contact export marks with
`use_this_point_for_hand_attachment == 1` may update the object-local anchor.
"""
