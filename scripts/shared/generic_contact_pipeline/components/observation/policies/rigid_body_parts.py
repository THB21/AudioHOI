...
def build(profile: CaseProfile) -> dict[str, object]:
    ...
    observation_seed_dir = profile.result_dir / "observation_seed"
    observation_seed_solver = REPO / "scripts/shared/generic_contact_pipeline/components/pose/solvers/mug_observation_seed.py"
    observation_seed_cmd = [
        runtime_python("audiohoi", override_env="AUDIOHOI_PYTHON"),
        str(observation_seed_solver),
        "--sample-dir", str(profile.sample_dir),
        "--observations-csv", str(observation_csv),
        "--proxy-csv", str(proxy_csv),
        "--out-dir", str(observation_seed_dir),
    ]
    subprocess.run(observation_seed_cmd, cwd=REPO, check=True)
    phase_source, phase_info = resolve_mug_m17_phase(profile)
    write_mug_m17_reconstruction_report(profile, phase_info)
    phase_by_frame = {
        str(int(float(row["frame"]))): row
    ...
    cmd = [
        ...
        str(phase_source),
        "--pose-csv",
        str(observation_seed_dir / "body_pose.csv"),
        "--out-dir",
        str(export_dir),
    ]
    ...
    metrics = {
        ...
        "observation_seed_pose": str(observation_seed_dir / "body_pose.csv"),
        "observation_seed_report": str(observation_seed_dir / "observation_seed_report.json"),
        "observation_seed_solver": str(observation_seed_solver),
