...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run generic pipeline ball anchor/contact depth refinement.")
    parser.add_argument("--sample-dir", type=Path, required=True)
    parser.add_argument("--body-model-root", type=Path, default=Path("third-party/GVHMR/inputs/checkpoints/body_models"))
    parser.add_argument("--out-subdir", type=str, default="pose6d_object_proxy_anchor_refined")
    ...
    state_csv = args.contact_state_csv or (results_dir / "contact_candidates_object_proxy" / "contact_state_frames.csv")
    event_csv = args.contact_event_csv or (results_dir / "contact_candidates_object_proxy" / "contact_candidates_labeled.csv")
    proxy_obs_csv = args.object_proxy_observation_csv or (results_dir / "object_proxy_observations" / "object_proxy_observations.csv")
    support_json = args.support_geometry_json or (results_dir / "pose6d_object_proxy_da3_init" / "support_geometry.json")
    pose_csv = args.object_pose_csv or (results_dir / "pose6d_object_proxy_da3_init" / "object_pose6d_sharedcam_trajectory.csv")
    ...
    contact_offset_raw = np.asarray(
        [proxy_offsets.get(frame, {}).get("contact_depth_offset_m", 0.0) for frame in object_frames],
        dtype=np.float64,
    )
    valid_contact_offset = np.isfinite(contact_offset_raw) & (np.abs(contact_offset_raw) <= args.max_contact_depth_offset_m)
        ...
        out_rows.append(
            {
                ...
                "contact_depth_offset_m": f"{contact_offset_raw[idx]:.6f}",
                "z_contact_final": f"{z_contact_final[idx]:.6f}",
                "contact_depth_offset_used_m": f"{contact_offset_for_anchor[idx]:.6f}",
    ...
    with summary_txt.open("w") as f:
        f.write("Generic contact pipeline ball anchor/contact depth refinement.\n")
        f.write("source_solver: generic_contact_pipeline.components.refinement.solvers.anchor_depth_solver\n")
