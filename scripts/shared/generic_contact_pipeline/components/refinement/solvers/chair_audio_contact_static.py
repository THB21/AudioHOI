...
def resolve_static_start(rows: list[dict[str, str]], place_frame: int, use_object_motion_gate: bool = False) -> tuple[int, str]:
    ...
    The chair pipeline has no reliable object-motion gate yet (unlike the mug
    one), so we fall back to the strongest late audio impact as the release
    event. Pose velocity is too noisy and can delay static by many frames.
    """
