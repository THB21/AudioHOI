...
# Solid rendering colors (BGR) matched to Articraft's warm sandy-beige ceramic appearance.
# Values are intentionally bright so that diffuse shading darkens to the right range.
MUG_SOLID_COLORS_BGR: dict[str, np.ndarray] = {
    # BGR channel order.  These bright warm values darken to sandy-beige after shading.
    'body_shell':  np.array([175.0, 205.0, 222.0]),   # BGR: B=175 G=205 R=222  → warm cream
    'rim_ring':    np.array([155.0, 183.0, 200.0]),   # slightly darker at rim
    'bottom_disk': np.array([145.0, 173.0, 190.0]),
    'handle_loop': np.array([175.0, 205.0, 222.0]),   # same warm cream as body
}

# Light direction = unit vector FROM surface TOWARD light source, in camera space.
# Camera looks in +z.  Light is at upper-left-front (between camera and scene) →
# z component is NEGATIVE (light is behind the camera, closer to viewer).
# x=-0.30 (from right of camera → surface lit on left), y=-0.55 (from above).
...
def draw_solid_mesh(
    ...
    for tri in faces:
        ...
        # Inward normals: z > 0 → front-facing (surface faces the camera).
        if n[2] < 0:
            continue
