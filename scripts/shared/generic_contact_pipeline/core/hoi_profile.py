...
from .vlm_provider import load_vlm_provider
...
def resolve_hoi_profile(profile: CaseProfile, mode: str = "seed") -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if mode == "qwen":
        ...
        script = REPO / "scripts/shared/generic_contact_pipeline/stages/stage_llm_qwen_profile.py"
        cmd = [
            str(provider.python),
            str(script),
            "--case",
    ...
    if mode == "mistral":
        seed_resolved, seed_trace = build_profile_from_seed(profile)
        script = REPO / "scripts/shared/generic_contact_pipeline/stages/stage_llm_mistral_profile.py"
        cmd = [
            sys.executable,
            str(script),
            "--case",
