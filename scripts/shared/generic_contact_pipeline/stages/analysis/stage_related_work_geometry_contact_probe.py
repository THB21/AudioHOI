...
def choose_local_points_path(case: str, variant: str) -> Path | None:
    ...
    for key in ["object_local_points_csv", "semantic_local_points_csv"]:
        ...
        if rel:
            ...
            if path.exists():
                return path
    # some baselines drop local points under final_result; the generic snapshot
    # uses the same object model, so it's a fine source for surface checks
...
def proxy_radius(case: str, pose_rows: list[dict[str, str]], local_pts: np.ndarray) -> float | None:
    ...
    if local_pts.size:
        ...
        if len(d):
            # conservative radius for root/object clearance. for chair this isn't
            # a collision result, just a guard against huge flips
...
def compute_variant(case: str, variant: str) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    # ball baselines keep the fullest contact/depth labels on the final pose CSV;
    # mug/chair keep semantic contact anchors in their contact tables
