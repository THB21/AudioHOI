...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify read-only stick line-contact diagnostics.")
    parser.add_argument("--case", default="stick")
    parser.add_argument("--result-name", default="benchmark_vlm_qwen")
    parser.add_argument("--out", type=Path)
    args = parser.parse_args()
    if args.case != "stick":
        raise SystemExit("line contact diagnostics currently supports --case stick")
    profile = with_runtime_overrides(load_case_profile(args.case), result_name=args.result_name)
    diagnostics = build_line_contact_diagnostics(profile.result_dir)
    ...
    print(
        f"stick: line_contact_diagnostics rows={summary['pose_rows']} "
        f"se3_refined={summary['se3_refined_frames']} "
