...
Layout:
    AudioHOI/
    ├── scripts/
    │   └── shared/human/gvhmr/run_gvhmr_local.py
    │   └── third-party/GVHMR/
    └── samples/<name>/
        ├── video.mp4
        └── results/gvhmr/
...
GVHMR_ROOT = (REPO_ROOT / "third-party" / "GVHMR").resolve()
GVHMR_CHECKPOINTS = GVHMR_ROOT / "inputs" / "checkpoints"
...
def ensure_required_checkpoints(static_cam: bool) -> None:
    required = [
        GVHMR_CHECKPOINTS / "yolo" / "yolov8x.pt",
        GVHMR_CHECKPOINTS / "vitpose" / "vitpose-h-multi-coco.pth",
        GVHMR_CHECKPOINTS / "gvhmr" / "gvhmr_siga24_release.ckpt",
        GVHMR_CHECKPOINTS / "hmr2" / "epoch=10-step=25000.ckpt",
        GVHMR_CHECKPOINTS / "body_models" / "smpl" / "SMPL_NEUTRAL.pkl",
        GVHMR_CHECKPOINTS / "body_models" / "smplx" / "SMPLX_NEUTRAL.npz",
    ]
    if not static_cam:
        required.append(GVHMR_CHECKPOINTS / "dpvo" / "dpvo.pth")

    missing = [p for p in required if not p.exists()]
    if missing:
        formatted = "\n".join(str(p) for p in missing)
        raise FileNotFoundError(
            "Missing GVHMR checkpoint/model files:\n"
            + indent(formatted, "  ")
            + "\n\nExpected them under: "
            + str(GVHMR_CHECKPOINTS)
        )
...
def run_gvhmr(
    ...
    ensure_required_checkpoints(static_cam)
