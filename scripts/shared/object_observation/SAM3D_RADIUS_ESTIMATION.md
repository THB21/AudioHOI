...
```bash
python scripts/shared/object_observation/estimate_radius_from_sam3d.py \
...
### Step 2: Use estimated radius in sharedcam

```bash
python scripts/shared/sharedcam/run_basketball_pose6d_sharedcam.py \
  --sample-dir samples/basketball_01 \
  --radius-estimates-csv results/object_observations/radius_estimates.csv
```

The script will:
1. Load per-frame radius estimates
2. Print mean estimated radius vs. default
3. Use estimated radius for depth initialization: `z = fx * radius_m / r_obs`
