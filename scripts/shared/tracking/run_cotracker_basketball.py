...
def mask_circle_row(mask_path: Path, frame_1based: int, fps: float) -> dict[str, object]:
    mask = cv2.imread(str(mask_path), cv2.IMREAD_GRAYSCALE)
    if mask is None:
        raise RuntimeError(f"Could not read mask {mask_path}")
...
def robust_center_from_tracks(tracks: np.ndarray, visibility: np.ndarray) -> tuple[float, float, str]:
    left_x, left_y = tracks[1]
    right_x, right_y = tracks[2]
    top_x, top_y = tracks[3]
    bottom_x, bottom_y = tracks[4]

    left_vis = float(visibility[1]) > 0.5
    right_vis = float(visibility[2]) > 0.5
    top_vis = float(visibility[3]) > 0.5
    bottom_vis = float(visibility[4]) > 0.5

    if left_vis and right_vis and top_vis and bottom_vis:
        cx = 0.5 * (left_x + right_x)
        cy = 0.5 * (top_y + bottom_y)
        return float(cx), float(cy), "cotracker_box_center"

    if left_vis and right_vis:
        cx = 0.5 * (left_x + right_x)
        cy = 0.5 * (left_y + right_y)
        return float(cx), float(cy), "cotracker_lr_center"

    if top_vis and bottom_vis:
        cx = 0.5 * (top_x + bottom_x)
        cy = 0.5 * (top_y + bottom_y)
        return float(cx), float(cy), "cotracker_tb_center"

    cx, cy = tracks[0]
    return float(cx), float(cy), "cotracker_center_fallback"
...
def main() -> None:
    ...
    for frame_idx in sorted(track_by_frame):
        ...
        cx, cy, source = robust_center_from_tracks(tracks, visibility)
        center_rows.append(
            {
                "frame": frame_1based,
    ...
    write_csv(tracking_dir / "cotracker_points.csv", point_rows, point_fields)
    ...
    for frame_idx in sorted(track_by_frame):
        frame_1based = frame_idx + 1
        mask_rows.append(mask_circle_row(masks_dir / f"{frame_1based:05d}_mask.png", frame_1based, args.fps))
    write_csv(
        tracking_dir / "ball_trajectory.csv",
        mask_rows,
        ["frame", "time", "ball_center_x", "ball_center_y", "radius", "mask_area", "source"],
    )
    print(f"frames: {len(center_rows)}")
    print(f"points: {POINT_NAMES}")
    print(f"trajectory: {tracking_dir / 'ball_trajectory.csv'}")
    print(f"cotracker_center_trajectory: {tracking_dir / 'cotracker_center_trajectory.csv'}")
    print(f"points_csv: {tracking_dir / 'cotracker_points.csv'}")
